...
def apply_fix_sascnvn():
    ...
    while i < len(lines):
        ...
        # 1. Add USE
        if 'use funcphys , only : fpvs' in line:
            out.append(line)
        ...
        # 2. Local variables
        if 'real(kind=kind_phys) ps(im),      del(im,km), prsl(im,km)' in line:
            out.append(line)
        ...
        if line.strip() == 'return' and i > len(lines) - 100: # Broaden search range
             # Find actual end of routine
...
def apply_fix_shalcnv():
    ...
    while i < len(lines):
        ...
        if 'real(kind=kind_phys) cincr, cincrmax, cincrmin' in line:
            out.append('      real(kind=kind_phys) mass_init_q1(im), mass_init_qlc(im),         &\n')
            out.append('     &                     mass_init_qli(im), zero_sink(im),            &\n')
            out.append('     &                     mnet(im,km)\n')
            out.append('      integer :: transport_opt = 1\n')
        ...
        if line.strip() == 'return' and i > len(lines) - 50:
            out.append('      if (use_mass_fixer) then\n')
            out.append('        call apply_mass_fixer(im, km, cnvflg, delp, zero_sink,          &\n')
            out.append('     &                        mass_init_q1, q1)\n')
            out.append('        call apply_mass_fixer(im, km, cnvflg, delp, zero_sink,          &\n')
