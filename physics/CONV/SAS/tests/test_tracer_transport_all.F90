program test_tracer_transport_all
  ...
  write(*,*) "| Transport Option | Initial Mass | Final Mass | Delta |"
  write(*,*) "|---:|---:|---:|---:|"

  do opt = 1, 3
    q_tracer = 1.0e-3_kind_phys
    if (opt == 3) q_tracer(1, 5) = 0.5e-10_kind_phys ! Cause clip
    if (opt == 2) q_tracer(1, 5) = -1.0e-4_kind_phys ! Cause hole fill
    ...
    case(1)
      ...
      write(*, '("| Flux-Form | ", F15.10, " | ", F15.10, " | ", E15.5, " |")') m_init(1), m_init(1), 0.0
    case(2)
      ...
      write(*, '("| Hole-Filling | ", F15.10, " | ", F15.10, " | ", E15.5, " |")') m_init(1), m_final(1), m_final(1) - m_init(1)
    case(3)
      call transport_tracer_advective(im, km, delt, cnvflg, delp, mnet, q_tracer)
      call calc_column_mass(im, km, cnvflg, q_tracer, delp, m_final)
      write(*, '("| Advective | ", F15.10, " | ", F15.10, " | ", E15.5, " |")') m_init(1), m_final(1), m_final(1) - m_init(1)
    end select
  enddo
  ...
  q_tracer = 1.0e-3_kind_phys
  call calc_column_mass(im, km, cnvflg, q_tracer, delp, m_init)
  q_tracer = q_tracer * 0.9_kind_phys
  call apply_mass_fixer(im, km, cnvflg, delp, rn_sink, m_init, q_tracer)
  call calc_column_mass(im, km, cnvflg, q_tracer, delp, m_final)
  write(*, '("| Mass-Fixer | ", F15.10, " | ", F15.10, " | ", E15.5, " |")') m_init(1), m_final(1), m_final(1) - m_init(1)
